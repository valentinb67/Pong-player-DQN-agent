...
for df, label in df_q_discret:
    df_filtered = df[df['Episode'] <= 1000]  # Filtrer les épisodes jusqu'à 100
...
for df, label in df_q_discret:
    ...
    if 'Touches Globales' in df_filtered.columns:
        axs[2].plot(df_filtered['Episode'], df_filtered['Touches Globales'], label=label, color=color_map[label])
axs[2].set_title("Evolution du Nombre de touches")
axs[2].set_xlabel("Épisode")
axs[2].set_ylabel("Nombre de touches cumulée")
axs[2].legend()
